...
func (handler *Handler) BuildImage(dockerfile, imageName string) error {
	...
	if err != nil {
		return traceExitOfError(err, "Failed to build Dockerfile tar")
	}
	ctx := context.Background()
...
func createDockerfileBuild(dockerfile string) (io.Reader, error) {
	...
	if err != nil {
		return nil, traceExitDockerfileBuildingError(err, dockerfile, "Failed to read Dockerfile to create tar")
	}
	...
	if err != nil {
		return nil, traceExitDockerfileBuildingError(err, dockerfile, "Failed to write header of tar file")
	}
	_, err = tw.Write(dockerfileBytes)
	if err != nil {
		return nil, traceExitDockerfileBuildingError(err, dockerfile, "Failed to write tar file")
	}
